# mindone/examples/sharegpt_4v/share4v/transformers/models/clip/modeling_ms_clip.py
# coding=utf-8
...
class CLIPTextEmbeddings(nn.Cell):
    def __init__(self, config: CLIPTextConfig):
        ...
        #cong TODO: better coidng style?
        self.standard_position_embeddings = 77
        self.token_embedding = nn.Embedding(config.vocab_size, embed_dim)
        self.position_embedding = nn.Embedding(self.standard_position_embeddings, embed_dim)
    ...
    def construct(
        ...
        num_inputs = input_ids.shape[0] if input_ids is not None else inputs_embeds.shape[0]

        sot = input_ids[:, :1]
        eot = input_ids[:, -1:]

        if position_ids is None:
            position_ids = ops.repeat_elements(self.position_ids[:, :seq_length], num_inputs, axis=0)

        # seq_length in (77,256)
        if position_ids.shape[-1] > self.standard_position_embeddings and \
                position_ids.shape[-1] < self.position_ids.shape[-1]:
            # devided by 75 and calculate the embeddings
            # m*75 + n
            m = (seq_length-2) // (self.standard_position_embeddings-2)
            n = (seq_length-2) % (self.standard_position_embeddings-2)

            embeddings = None
            position_ids_slice =  ops.repeat_elements(ops.arange(self.standard_position_embeddings).unsqueeze(0),
                                                      num_inputs, axis=0)

            for index in range(m+1):
                # get start of tokens and end of tokens
                print("index", index)
                if index == m:
                    input_ids_slice = ops.concat((sot, input_ids[:, 1 + index*75: 1 + index* 75 + n],
                                                  eot), axis=1)

                    position_ids_slice = ops.repeat_elements(ops.arange(n+2).unsqueeze(0), num_inputs)
                else:
                    input_ids_slice = ops.concat((sot, input_ids[:, 1 + index*75: 1 + (index+1) * 75],
                                                  eot), axis=1)

                if inputs_embeds is None:
                    inputs_embeds_slice  = self.token_embedding(input_ids_slice)
                else:
                    raise NotImplementedError
                position_embeddings_slice  = self.position_embedding(position_ids_slice)
                embeddings_slice = inputs_embeds_slice + position_embeddings_slice


                if embeddings is None:
                    embeddings = embeddings_slice
                else:
                    embeddings = ops.concat((embeddings[:,:-1], embeddings_slice[:,1:]), axis=1)
...
class CLIPTextTransformer(nn.Cell):
    ...
    def construct(
        ...
        print("-"*40)
        print("CLIPTextTransformer")
        print("input_ids.shape", input_ids.shape) #(4, 3)
        if position_ids is not None: print("position_ids.shape", position_ids.shape)  # (4, 3)
        print("hidden_states.shape", hidden_states.shape)  # (4, 3, 512)
